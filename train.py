...
def train(config, module):
    ...
    train_set, train_ensemble_set, val_set, val_final_set, test_set = get_base_datasets(
        config
    )
    ...
    val_loader = DataLoader(val_set, batch_size=TEST_BATCH_SIZE, num_workers=1)
    ...
    if config["nepochs"] == 0:
        ...
        assert (not config["debug"]) and (
            config["full_data"]
        ), "log only if using full data"
    else:
    ...
    run_eval(
        model,
        ckpt_path=ckpt_path,
        train_ensemble_set=train_ensemble_set,
        val_set=val_set,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log_only",
        action="store_true",
        help="only evaluate and log to wandb, skip training. useful for pretrained HF models",
    )
