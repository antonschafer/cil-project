...
DEBUG_TRAINER_ARGS = {"limit_train_batches": 10, "limit_val_batches": 5}
...
MODELS = {
    ...
    "twitter_roberta": {
        ...
        "data_transform": lambda x: x.replace("<user>", "@user").replace(
            "<url>", "http"
        ),
    },
    ...
    "twitter_roberta_embeddings": {
        ...
        "module": lambda _: AutoModel.from_pretrained(
    ...
    "twitter_roberta_pos_neut_neg_preds": {
        "model_name": "cardiffnlp/twitter-roberta-base-sentiment",
        "tokenizer_name": "cardiffnlp/twitter-roberta-base-sentiment",
        "module": lambda _: AutoModelForSequenceClassification.from_pretrained(
...
def get_base_datasets(config):
    ...
    option_str = "_".join(
        [
            config["tokenizer_name"].split("/")[-1],
            str(config["full_data"]),
            str(function_to_hash(data_transform)),
            str(config["seed"]),
            str(text_with_prompt),
            str(config["train_data_size"]),
            "v" + str(DATA_VERSION),
        ]
    )
    ...
    if os.path.exists(cache_file):
        ...
        train_ensemble_data = BaseDataset(
        ...
        write_pickle(
            [train_data, train_ensemble_data, val_data, val_final_data, test_data],
            cache_file,
        )
...
def merge_metrics():
    ...
    for i, filename in enumerate(os.listdir("statistics")):
        ...
        cov = pd.DataFrame(
            [
                [
                    ...
                    None,
                    None,
                    pd.read_csv(os.path.join("statistics", filename)).mean()[0],
                ]
            ]
        )
        ...
        for j, filename2 in enumerate(os.listdir("statistics")):
            ...
            comb = (
                pd.read_csv(os.path.join("statistics", filename)).iloc[:, 0]
                | pd.read_csv(os.path.join("statistics", filename2)).iloc[:, 0]
            )
            cov = pd.DataFrame([[filename, filename2, None, comb.mean()]])
            cov.to_csv(
                os.path.join("statistics", "coverage.csv"), header=False, mode="a"
            )
            ...
            for k, filename3 in enumerate(os.listdir("statistics")):
                ...
                combs = (
                    comb | pd.read_csv(os.path.join("statistics", filename3)).iloc[:, 0]
                )
...
def get_trainer(config):
    ...
    callbacks = [
        EarlyStopping(monitor="val_loss", mode="min", patience=config["es_patience"]),
        ModelCheckpoint(
            monitor="val_loss", dirpath=config["save_dir"], filename="model"
        ),
    ]
...
def get_base_arg_parser():
    ...
    parser.add_argument(
        "--es_patience", type=int, default=3, help="early stopping patience"
    )

    parser.add_argument("--full_data", action="store_true")
    parser.add_argument("--debug", action="store_true", help="only run a few batches")
...
def load_wandb_file(fname, run_id, save_dir):
    ...
    return wandb.restore(
        fname, run_path=WANDB_PROJECT_PATH + run_id, root=run_cache_dir
    ).name
