...
def load_data(full_data):
    ...
    train_filename = (
        "./twitter-datasets/"
        + ("full_" if full_data else "small_")
        + "train_v{}.csv".format(DATA_VERSION)
    )
    val_filename = (
        "./twitter-datasets/"
        + ("full_" if full_data else "small_")
        + "val_v{}.csv".format(DATA_VERSION)
    )
    valfinal_filename = (
        "./twitter-datasets/"
        + ("full_" if full_data else "small_")
        + "val_final_v{}.csv".format(DATA_VERSION)
    )
...
def run_glove(config):
    ...
    maxlen = 104
    train_data_glove = tf.keras.preprocessing.sequence.pad_sequences(
        train_data_glove, maxlen=maxlen
    )
    val_data_glove = tf.keras.preprocessing.sequence.pad_sequences(
        val_data_glove, maxlen=maxlen
    )
    val_final_data_glove = tf.keras.preprocessing.sequence.pad_sequences(
        val_final_data_glove, maxlen=maxlen
    )
    test_data_glove = tf.keras.preprocessing.sequence.pad_sequences(
        test_data_glove, maxlen=maxlen
    )

    model = tf.keras.Sequential()

    model.add(Bidirectional(LSTM(100, return_sequences=True), input_shape=(104, 100)))
    ...
    model.fit(
        ...
        epochs=5,
        callbacks=[callback],
        validation_data=(val_data_glove, val_data["labels"]),
        verbose=2,
    )
...
def run_tfidf(config):
    ...
    clf = LogisticRegression(random_state=0, max_iter=100000).fit(
        train_data_tf, train_data["labels"]
    )
    ...
    wandb.log(
        {"train_acc": train_acc, "val_acc": val_acc, "val_final_acc": val_final_acc}
    )
